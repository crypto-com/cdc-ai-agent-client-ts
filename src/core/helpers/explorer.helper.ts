import { CHAIN_URLS } from '../blockchain/blockchain.constants.js';
...
export const getBaseUrl = (chain: string): string => {
  ...
  return CHAIN_URLS[chainType] || 'Chain does not exist in the Cronos Ecosystem';
};
...
const validateChainType = (name: string): ChainType => {
  ...
  throw new Error(`Invalid chain name: ${name}. Must be one of ${Object.values(ChainType).join(', ')}`);
};
